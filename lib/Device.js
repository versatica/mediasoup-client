...
export default class Device
{
	...
	load({ roomRtpCapabilities } = {})
	{
		...
		if (!roomRtpCapabilities)
			return Promise.reject(new Error('missing roomRtpCapabilities'));
	 ...
	 * @params {Object} remoteParameters - Parameters of the associated server-side
	 *   transport.
	 * @param {Object} [options]
	 * @param {Array<RTCIceServer>} [options.turnServers] - Array of TURN servers.
	 * @param {RTCIceTransportPolicy} [options.iceTransportPolicy] - ICE transport
	 *   policy.
	 *
	 * @return {Transport}
	 *
	 ...
	 * @throws {InvalidStateError} if not loaded.
	 */
	createTransport(direction, remoteParameters, options = {})
	{
		...
		const transport = new Transport(
			direction, remoteParameters, options, this._extendedRtpCapabilities);
