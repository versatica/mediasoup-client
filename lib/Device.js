...
class Device
{
	...
	load({ routerRtpCapabilities } = {})
	{
		...
		if (this._loaded)
			...
			return Promise.reject(new TypeError('missing routerRtpCapabilities'));
	...
	/**
	 * Whether we can consumer a producer.
	 *
	 ...
	 * @throws {TypeError} if wrong arguments.
	 */
	canConsume(consumableRtpParameters)
	{
		if (!this._loaded)
			throw new InvalidStateError('not loaded');
		else if (typeof consumableRtpParameters !== 'object')
			throw new TypeError('missing consumableRtpParameters');
