...
export default class Device
{
	...
	static _detect()
	{
		...
		else if (global.navigator && typeof global.navigator.userAgent === 'string')
		{
			...
			else if (bowser.check({ chrome: '49', chromium: '49' }, true, ua) && global.adapter)
			{
				Device._flag = 'chrome';
				Device._handlerClass = Chrome55;
			}
			...
			else if (bowser.check({ firefox: '65' }, true, ua))
			{
				...
				Device._handlerClass = Firefox65;
			}
			// Firefox (desktop and mobile).
			...
			{
				logger.warn('best effort browser detection [name:"%s"]', browser.name);
