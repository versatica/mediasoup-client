...
class Transport extends EnhancedEventEmitter
{
	 ...
	 * @emits {consumerLocalParameters: Object, callback: Function, errback: Function} consume
	 ...
	 */
	consume({ producerId, preferredSpatialLayer, appData } = {})
	{
		logger.debug('consume()');

		if (this._closed)
			return Promise.reject(new InvalidStateError('transport closed'));
		else if (!producerId || typeof producerId !== 'string')
		...
		else if (!utils.isValidSpatialLayer(preferredSpatialLayer, { optional: true }))
			return Promise.reject(new TypeError('invalid preferredSpatialLayer'));
		else if (appData && typeof appData !== 'object')
			return Promise.reject(new TypeError('if given, appData must be an object'));
					...
					.then(() =>
					{
						return this.safeEmitAsPromise(
							...
							// consumerLocalParameters.
							{
								...
								preferredSpatialLayer : preferredSpatialLayer || undefined,
								appData
							});
					})
