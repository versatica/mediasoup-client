...
export default class Transport extends EnhancedEventEmitter
{
		...
		return Promise.resolve()
			...
			.then((rtpParameters) =>
			{
				return new Promise((resolve, reject) =>
				{
					...
					this.safeEmit('request', 'createReceiver', data, resolve, reject);
				});
			})
			.then(() =>
			{
	...
	_execAddReceiver(receiver)
	{
		...
		// TODO: Should we check here whether we can receive the remote track
		// based on our RTP capabilities?
	...
	_handleHandler()
	{
		...
		handler.on('needupdatetransport', (transportLocalParameters) =>
		{
			...
			this.safeEmit('notification', 'updateTransport', data);
		});
	}
