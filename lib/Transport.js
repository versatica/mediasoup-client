...
export default class Transport extends EnhancedEventEmitter
{
	...
	)
	{
		...
		this._effectiveRtpCapabilities = effectiveRtpCapabilities;
	...
	send({ track, simulcast, appData } = {})
	{
		...
		const producer = new Producer({ track, simulcast, appData });
				...
				return Promise.resolve()
					.then(() =>
					{
						return this.safeEmitAsPromise(
							...
							{
								producerId,
								rtpCapabilities : this._extendedRtpCapabilities,
								appData
							});
					})
					.then((remoteConsumerData) =>
					{
						consumer = new Consumer({ remoteConsumerData, appData });
	...
	_handleConsumer(consumer)
	{
		consumer.on('@close', () =>
		{
			...
			this._commandQueue.push(
				() => this._handler.removeConsumer(consumer))
				.catch(() => {});
		});
	}
}
