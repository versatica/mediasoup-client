...
export default class Transport extends EnhancedEventEmitter
{
	...
	async restartIce()
	{
		...
		try
		{
			...
			const { remoteIceParameters } = await this.safeEmitAsPromise('@request', 'restartTransport', data);
	...
	async _execAddProducer(producer)
	{
		logger.debug('_execAddProducer()');

		// Call the handler.
		const producerRtpParameters = await this._handler.addProducer(producer);
		...
		await this.safeEmitAsPromise('@request', 'createProducer', data);
	...
	async _execAddConsumer(consumer)
	{
		logger.debug('_execAddConsumer()');

		// Call the handler.
		const consumerTrack = await this._handler.addConsumer(consumer);
		...
		const { paused, preferredProfile, effectiveProfile } = await this.safeEmitAsPromise('@request', 'enableConsumer', data);
