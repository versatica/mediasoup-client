...
export default class Transport extends EnhancedEventEmitter
{
	...
	_execAddReceiver(receiver)
	{
		...
		return Promise.resolve()
			...
			.then((track) =>
			{
				...
				// Tell the server that we want to receive the associated remote Sender.
	...
	{
		// If this Transport is closed, notify it to the Sender.
		this.on('close', () =>
		{
			if (!sender.closed)
				sender.setHandled(false);
	...
	{
		// If this Transport is closed, notify it to the Receiver.
		this.on('close', () =>
		{
			if (!receiver.closed)
				receiver.setHandled(false);
