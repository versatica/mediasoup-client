...
/**
 * Generates receiving settings based on given RTP parameters and RTP capabilities.
 *
 * @param {RTCRtpParameters} rtpParameters
 * @param {RTCExtendedRtpCapabilities} extendedRtpCapabilities
 * @return {Object}
 */
export function getReceivingSettings(rtpParameters, extendedRtpCapabilities)
{
	const settings =
	{
		useRtx : false
	...
	const firstMediaCodec = rtpParameters.codecs[0];
	const matchingCapCodec = extendedRtpCapabilities.codecs
		.find((codec) => codec.recvPayloadType === firstMediaCodec.payloadType);

	if (matchingCapCodec.recvRtxPayloadType)
