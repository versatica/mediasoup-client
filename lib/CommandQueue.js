...
export default class CommandQueue extends EventEmitter
{
	...
	push(command)
	{
		logger.debug('push() [command.name:%s]', command.name);
	...
	{
		logger.debug('_handleCommand() [command.name:%s]', command.name);
		...
		return promiseHolder.promise
			.then((result) =>
			{
				logger.debug('_handleCommand() succeeded [command.name:%s]', command.name);
			...
			.catch((error) =>
			{
				logger.error('_handleCommand() failed [command.name:%s]: %o',
					command.name, error);
