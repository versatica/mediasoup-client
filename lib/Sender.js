...
export default class Sender extends EnhancedEventEmitter
{
	...
	/**
	 * Whether the Sender is locally paused.
	 *
	 * @return {Boolean}
	 */
	get locallyPaused()
	...
	/**
	 * Whether the Sender is remotely paused.
	 *
	 * @return {Boolean}
	 */
	get remotelyPaused()
	...
	/**
	 * Whether the Sender is actually sending media.
	 *
	 * @return {Boolean}
	 */
	get active()
	...
	/**
	 * Destroys the Sender and stops the cloned track.
	 *
	 * @private
	 */
	destroy()
	...
	pause(appData)
	{
		...
		if (this._closed)
		{
			logger.error('pause() | Sender closed');

			return false;
		}
		else if (!this._handled)
		{
			logger.error('pause() | Sender not handled');
	...
	resume(appData)
	{
		...
		if (this._closed)
		{
			logger.error('resume() | Sender closed');
	...
	/**
	 * My remote Receiver was resumed.
	 * Invoked via remote notification.
	 *
	 * @private
	 * @param {Any} [appData] - App custom data.
