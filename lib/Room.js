...
export default class Room extends EnhancedEventEmitter
{
		...
		try
		{
			...
			switch (method)
			{
				...
				case 'peerClosed':
				{
					...
					this._handlePeerClosed(peer, appData);
				...
				case 'newSender':
				{
					...
					this._handleNewReceiver(senderData, peer);
				...
				case 'senderClosed':
				{
					...
					this._handleReceiverClosed(senderData, peer);
				...
				case 'senderPaused':
				{
					...
					this._handleReceiverPaused(senderData, peer);
				...
				case 'senderResumed':
				{
					...
					this._handleReceiverResumed(senderData, peer);
	...
	_handlePeerClosed(peer, appData)
	{
		peer.close(appData);
	}

	_handleNewReceiver(senderData, peer)
	{
		const { id, kind, rtpParameters, paused, appData } = senderData;
		const receiver = new Receiver(id, kind, rtpParameters, appData);
		const supported =
			ortc.canReceive(receiver.rtpParameters, this._extendedRtpCapabilities);
	...
	_handleReceiverClosed(senderData, peer)
	{
		const { id, appData } = senderData;

		peer.closeReceiver(id, appData);
	}

	_handleReceiverPaused(senderData, peer)
	{
		const { id, appData } = senderData;

		peer.pauseReceiver(id, appData);
	}

	_handleReceiverResumed(senderData, peer)
	{
		const { id, appData } = senderData;

		peer.resumeReceiver(id, appData);
	}
}
