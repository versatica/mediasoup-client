...
export default class Room extends EnhancedEventEmitter
{
	...
	receiveNotification(notification)
	{
		try
		{
			...
			switch (method)
			{
				...
				{
					const { id, appData } = notification;
					const sender = this._senders.get(id);

					if (!sender)
						throw new Error(`Sender not found [id:${id}]`);

					sender.remoteClose(appData);
				...
				case 'receiverPaused':
				{
					...
					sender.remotePause(appData);

					break;
				}

				case 'receiverResumed':
				{
					const { id, appData } = notification;
					const sender = this._senders.get(id);
				...
				case 'newSender':
				{
					...
					this._handleNewReceiverData(senderData, peer);
	...
	_sendRequest(method, data)
	{
		...
		return new Promise((resolve, reject) =>
		{
			...
			// TODO: We could also handle room 'closed'/'left' event here.
	...
	_handleNewReceiverData(senderData, peer)
	{
