...
export default class Room extends EnhancedEventEmitter
{
	...
	join()
	{
		...
		return Device.Handler.getLocalRtpCapabilities()
			.then((rtpCapabilities) =>
			{
				...
				return this._sendRequest('join', { appData: this._appData });
			})
			.then((response) =>
			{
	...
	receiveNotification(notification)
	{
		...
		switch (method)
		{
			case 'newReceiver':
			{
				// TODO: Wrong, we must check the peerId of the notification and match
				// an existing peer, and we need to handle peers, etc.

				const receiver = new Receiver(
					notification.receiverId, notification.rtpParameters,
					notification.appData);

				if (this._receivers.has(receiver.id))
				{
					logger.error(
						'receiveNotification() | already have a Receiver with same id');
			...
			default:
				logger.error(
					'receiveNotification() | unknown notification [method:%s]', method);
		}
	}
	...
	createSender(track)
	{
		...
		// Create a new Sender.
		const sender = new Sender(track, originalTrack);
