...
export default class Room extends EnhancedEventEmitter
{
		...
		try
		{
			...
			switch (method)
			{
				...
				case 'newReceiver':
				{
					...
					const receiverData = notification;

					this._handleNewReceiver(receiverData, peer);
	...
	{
		const { name, receivers, appData } = peerData;
		...
		// Add Receivers.
		for (let receiverData of receivers)
		{
			try
			{
				this._handleNewReceiver(receiverData, peer);
			...
			{
				logger.error('error handling existing Receiver:%o', error);
			}
		}
	...
	_handleNewReceiver(receiverData, peer)
	{
		const { id, kind, rtpParameters, paused, appData } = receiverData;
		const receiver = new Receiver(id, kind, rtpParameters, appData);

		if (paused)
			receiver.setPaused(true);

		if (!ortc.canReceive(receiver.rtpParameters, this._extendedRtpCapabilities))
			receiver.setSupported(false);
