...
export default class Room extends SafeEventEmitter
{
	...
	/**
	 * Join the remote room with the given participant name.
	 *
	...
	_execCommand(command, promiseHolder)
	{
		...
		switch (command.name)
		{
			case 'join':
			{
				const { peerName } = command.data;

				promise = this._execJoin(peerName);
				break;
			}
			...
			default:
			{
				promise = Promise.reject(new Error(`unknown command.name "${command.name}"`));
			}
		}
