...
export default class Room extends SafeEventEmitter
{
	...
	_execJoin()
	{
		...
		{
			this._handler.getLocalRtpCapabilities()
				.then((localRtpCapabilities) =>
				{
					const command =
					{
						name : 'join',
	...
	_execAddLocalTrack(track)
	{
		...
		return this._handler.addLocalTrack(track)
			.then((rtpParameters) =>
			{
				...
				// Send a command to the server telling about the new Sender.
				const command =
				{
					name : 'newSender',
					data : { rtpParameters }
				...
				const errback = (message) =>
				{
					...
					sender.close(new Error(message));
				};

				this._emitCommand(command, null, errback);
