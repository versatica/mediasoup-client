...
export default class Room extends EnhancedEventEmitter
{
	 ...
	 * @return {Promise}
	 */
	join(roomSettings, appData)
	{
		if (!roomSettings)
			logger.debug('join()');
		else
			logger.debug('join() [roomSettings:%o]', roomSettings);
		...
		return Promise.resolve()
			...
			.then((rtpCapabilities) =>
			{
				...
				logger.debug(
					'join() | local RTP capabilities:%o', localRtpCapabilities);
				...
				const unsupportedRoomCodecs = utils.getUnsupportedCodecs(
