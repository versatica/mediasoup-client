...
class SendHandler extends Handler
{
	...
	{
		logger.debug(
			'sendDataChannel() [ordered:%s, maxPacketLifeTime:%s, maxRetransmits:%s, priority:%s]',
			ordered, maxPacketLifeTime, maxRetransmits, priority);

		const options =
		{
			negotiated        : true,
			id                : this._nextDataChannelId,
			ordered,
			maxPacketLifeTime,
		...
		const dataChannel =
			this._pc.createDataChannel(`dc${this._nextDataChannelId}`, options);

		// Increase next id.
		// NOTE: Chrome max id is 1023.
		if (this._nextDataChannelId < 1023)
			this._nextDataChannelId++;
		else
			this._nextDataChannelId = 0;
		...
		const sctpStreamParameters =
		{
			...
			maxPacketLifeTime : dataChannel.maxPacketLifeTime || dataChannel.maxRetransmitTime,
...
class RecvHandler extends Handler
{
	...
	{
		logger.debug(
			'receiveDataChannel() [sctpStreamParameters:%o]', sctpStreamParameters);
		...
		{
			options.maxPacketLifeTime = maxPacketLifeTime;
			options.maxRetransmitTime = maxPacketLifeTime; // NOTE: Old spec.
		}
		...
		const dataChannel =
			this._pc.createDataChannel(`dc${streamId}`, options);
