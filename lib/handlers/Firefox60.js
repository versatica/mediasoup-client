...
class SendHandler extends Handler
{
	...
	async send({ track, encodings, codecOptions })
	{
		...
		const transceiver = this._pc.addTransceiver(track, { direction: 'sendonly' });
		...
		if (encodings)
		{
			...
			await transceiver.sender.setParameters({ ...parameters, encodings });
		}
		...
		sendingRtpParameters.rtcp.cname =
			sdpUnifiedPlanUtils.getCname({ offerMediaObject });

		// Set RTP encodings just if no simulcast is used.
		if (!encodings || encodings.length <= 1)
		{
			sendingRtpParameters.encodings =
				sdpUnifiedPlanUtils.getRtpEncodings({ offerMediaObject });
		}
		// Otherwise use the given encodings verbatim.
		else
		{
...
class Firefox60
{
	static async getNativeRtpCapabilities()
	{
		...
		try
		{
			...
			await videoTransceiver.sender.setParameters({ ...parameters, encodings });
