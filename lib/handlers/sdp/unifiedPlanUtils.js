...
exports.fillRtpParametersForTrack = function(
	rtpParameters, sdpObj, track, { mid = null, legacySimulcast = false } = {}
)
{
	...
	if (!legacySimulcast)
	{
		...
		// Array of Objects with rid and spatialLayer keys.
		...
		for (const rid of mSection.rids || [])
		{
			...
			if (/^low/.test(rid.id))
				simulcastStreams.push({ rid: rid.id, spatialLayer: 'low' });
			else if (/^medium/.test(rid.id))
				simulcastStreams.push({ rid: rid.id, spatialLayer: 'medium' });
			if (/^high/.test(rid.id))
				simulcastStreams.push({ rid: rid.id, spatialLayer: 'high' });
		}
		...
		else
		{
			for (const simulcastStream of simulcastStreams)
			{
				const encoding =
				{
					rid          : simulcastStream.rid,
					spatialLayer : simulcastStream.spatialLayer
				};
	...
	else
	{
		...
		const simulcastSpatialLayers = [ 'low', 'medium', 'high' ];
		...
		for (const [ ssrc, rtxSsrc ] of ssrcToRtxSsrc)
		{
			...
			if (simulcast)
				encoding.spatialLayer = simulcastSpatialLayers.shift();
