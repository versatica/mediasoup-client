...
export default class Edge11 extends EnhancedEventEmitter
{
	...
	close()
	{
		...
		try { this._iceGatherer.close(); } catch (error) {}

		// Close the ICE transport.
		try { this._iceTransport.stop(); } catch (error) {}

		// Close the DTLS transport.
		try { this._dtlsTransport.stop(); } catch (error) {}

		// Close RTCRtpSenders.
		for (let rtpSender of this._rtpSenders.values())
		{
			try { rtpSender.stop(); } catch (error) {}
		}

		// Close RTCRtpReceivers.
		for (let rtpReceiver of this._rtpReceivers.values())
		{
			try { rtpReceiver.stop(); } catch (error) {}
		}
	}
