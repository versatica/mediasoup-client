...
class RecvHandler extends Handler
{
	constructor(rtpParametersByKind, settings)
	{
		...
		// Map of Receivers related info indexed by receiver.id.
		// - kind {String}
		// - trackId {String}
		// - ssrc {Number}
		// - rtxSsrc {Number}
	...
	addReceiver(receiver)
	{
		...
		const receiverInfo = RemotePlanBSdp.getReceiverInfo(receiver);

		console.warn(receiverInfo);
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				const rtpParameters = utils.clone(this._rtpParametersByKind[receiver.kind]);
