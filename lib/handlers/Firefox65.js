...
class Handler extends EnhancedEventEmitter
{
	...
	updateIceServers({ iceServers }) // eslint-disable-line no-unused-vars
	{
		...
		// NOTE: Firefox 65 does not implement pc.setConfiguration().

		return Promise.reject(
			new UnsupportedError('not implemented in this browser'));
	}
	...
	send({ track, simulcast })
	{
		...
		return Promise.resolve()
			...
			.then(() =>
			{
				...
				return sender.setParameters(newParameters);
			})
