...
class SendHandler extends Handler
{
	...
	addProducer(producer)
	{
		...
		return Promise.resolve()
			.then(() =>
			{
				// Let's check if there is any stopped transceiver for same kind and
				// reuse it if so.
			...
			.then((offer) =>
			{
				...
				if (producer.simulcast)
				{
					...
					sdpUnifiedPlanUtils.addPlanBSimulcastForTrack(sdpObject, track);
			...
			.then(() =>
			{
				...
				sdpUnifiedPlanUtils.fillRtpParametersWithPlanBSimulcastForTrack(
		...
		return Promise.resolve()
			.then(() =>
			{
				const transceiver = this._pc.getTransceivers()
					.find((t) => (t.sender && t.sender.track && t.sender.track.id === track.id));

				if (!transceiver)
					throw new Error('RTCRtpTransceiver not found');

				// Stop the transceiver.
				transceiver.stop();

				// TODO: Safari 12.1 does not let changes to direction attribute once
				// transceiver already in use.
...
class RecvHandler extends Handler
{
	...
	addConsumer(consumer)
	{
		...
		return Promise.resolve()
			...
			{
				const transceiver = this._pc.getTransceivers()
					.find((t) => (t.receiver && t.receiver.track && t.mid === consumerInfo.mid));
