...
export declare class Chrome67 extends HandlerInterface {
    ...
    send({ track, encodings, codecOptions, codec, localDtlsRole }: HandlerSendOptions): Promise<HandlerSendResult>;
    ...
    sendDataChannel({ ordered, maxPacketLifeTime, maxRetransmits, label, protocol, priority, localDtlsRole }: HandlerSendDataChannelOptions): Promise<HandlerSendDataChannelResult>;
    receive({ trackId, kind, rtpParameters, localDtlsRole }: HandlerReceiveOptions): Promise<HandlerReceiveResult>;
    stopReceiving(localId: string): Promise<void>;
    getReceiverStats(localId: string): Promise<RTCStatsReport>;
    receiveDataChannel({ sctpStreamParameters, label, protocol, localDtlsRole }: HandlerReceiveDataChannelOptions): Promise<HandlerReceiveDataChannelResult>;
