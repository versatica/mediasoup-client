...
class SendHandler extends Handler
{
		...
		})
	{
		...
		logger.debug('DataChanel options:%o', options);

		const dataChannel =
			this._pc.createDataChannel(`dc-out-${this._nextDataChannelId}`, options);
		...
		const sctpStreamParameters =
		{
			...
			maxPacketLifeTime : options.maxPacketLifeTime || options.maxRetransmitTime,
...
class RecvHandler extends Handler
{
	...
	async receiveDataChannel({ sctpStreamParameters })
	{
		...
		const dataChannel =
			this._pc.createDataChannel(`dc-in-${streamId}`, options);
