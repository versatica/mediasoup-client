	...
	addProducer(producer)
	{
		...
		return Promise.resolve()
			.then(() =>
			{
				...
				// If simulcast is enabled, add it.
				if (producer.simulcast)
				{
					const rid = Math.round(utils.randomNumber() / 100000);

					rtpSender.setParameters(
						{
							encodings :
							[
								{ rid: `low-${rid}`, maxBitrate: producer.simulcast.low },
								{ rid: `medium-${rid}`, maxBitrate: producer.simulcast.medium },
								{ rid: `high-${rid}`, maxBitrate: producer.simulcast.high }
							]
						});
				}
