...
exports.getExtendedRtpCapabilities = function(localCaps, remoteCaps)
{
	...
	for (const extendedCodec of extendedRtpCapabilities.codecs || [])
	{
		const matchingLocalRtxCodec = (localCaps.codecs || [])
			.find((localCodec) => (
				localCodec.name.toLowerCase() === 'rtx' &&
				localCodec.parameters.apt === extendedCodec.localPayloadType
			));

		const matchingRemoteRtxCodec = (remoteCaps.codecs || [])
			.find((remoteCodec) => (
				remoteCodec.name.toLowerCase() === 'rtx' &&
...
exports.getReceivingFullRtpParameters = function(kind, extendedRtpCapabilities)
{
	...
	for (const extendedCodec of extendedRtpCapabilities.codecs)
	{
		...
		const codec =
		{
			...
			mimeType     : extendedCodec.mimeType,
			clockRate    : extendedCodec.clockRate,
			payloadType  : extendedCodec.remotePayloadType,
			channels     : extendedCodec.channels,
			rtcpFeedback : extendedCodec.rtcpFeedback,
			parameters   : extendedCodec.remoteParameters
		};
		...
		if (extendedCodec.remoteRtxPayloadType)
		{
			const rtxCodec =
			{
				...
				mimeType    : `${extendedCodec.kind}/rtx`,
				clockRate   : extendedCodec.clockRate,
				payloadType : extendedCodec.remoteRtxPayloadType,
				parameters  :
				{
					apt : extendedCodec.remotePayloadType
				}
