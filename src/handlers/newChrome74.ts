...
import {
	...
	ReceiveOptions,
	ReceiveResult,
	SendDataChannelOptions,
	SendDataChannelResult,
	ReceiveDataChannelOptions,
	ReceiveDataChannelResult
...
export class Chrome74 extends HandlerInterface
{
	...
	// Remote SDP handler.
	private readonly _remoteSdp: RemoteSdp;

	// Generic sending RTP parameters for audio and video.
	private readonly _sendingRtpParametersByKind: { [key: string]: RtpParameters };

	// Generic sending RTP parameters for audio and video suitable for the SDP
	// remote answer.
	private readonly _sendingRemoteRtpParametersByKind: { [key: string]: RtpParameters };

	// RTCPeerConnection instance.
	private readonly _pc: any;

	// Map of RTCTransceivers indexed by MID.
	private readonly _mapMidTransceiver: Map<string, any> = new Map();
	...
	{
		return (options: HandlerOptions): Chrome74 => new Chrome74(options);
	}
	...
	): Promise<SendResult>
	{
		...
		const sendId = transceiver.mid;

		// Set MID.
		sendingRtpParameters.mid = sendId;
		...
		// Store in the map.
		this._mapMidTransceiver.set(sendId, transceiver);
	...
	async stopSending(sendId: string): Promise<void>
	{
		logger.debug('stopSending() [sendId:%s]', sendId);

		this.assertSendDirection();

		const transceiver = this._mapMidTransceiver.get(sendId);
	...
	async replaceTrack(sendId: string, track: MediaStreamTrack): Promise<void>
	{
		logger.debug('replaceTrack() [sendId:%s, track.id:%s]', sendId, track.id);
	...
	async setMaxSpatialLayer(sendId: string, spatialLayer: number): Promise<void>
	{
		logger.debug(
			'setMaxSpatialLayer() [sendId:%s, spatialLayer:%s]',
			sendId, spatialLayer);
	...
	async setRtpEncodingParameters(sendId: string, params: any): Promise<void>
	{
		logger.debug(
			'setRtpEncodingParameters() [sendId:%s, params:%o]',
			sendId, params);
	...
	async getSenderStats(sendId: string): Promise<any>
	{
		this.assertSendDirection();
	...
	async receive(
		{ trackId, kind, rtpParameters }: ReceiveOptions
	): Promise<ReceiveResult>
	{
		...
		const recvId = String(this._mapMidTransceiver.size);
		...
		const answerMediaObject = localSdpObject.media
			.find((m: any) => String(m.mid) === recvId);
		...
		const transceiver = this._pc.getTransceivers()
			.find((t: any) => t.mid === recvId);

		if (!transceiver)
			throw new Error('new RTCRtpTransceiver not found');

		// Store in the map.
		this._mapMidTransceiver.set(recvId, transceiver);
	...
	async stopReceiving(recvId: string): Promise<void>
	{
		logger.debug('stopReceiving() [recvId:%s]', recvId);

		this.assertRecvDirection();

		const transceiver = this._mapMidTransceiver.get(recvId);
	...
	async getReceiverStats(recvId: string): Promise<RTCStatsReport>
	{
		this.assertRecvDirection();
	...
	async receiveDataChannel(
		{ sctpStreamParameters, label, protocol }: ReceiveDataChannelOptions
