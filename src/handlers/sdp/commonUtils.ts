...
export function removeCodec(
	...
	for (const m of media) {
		...
		let codecPayload: number | undefined;

		// Retrieve the codec payload.
		for (const rtp of m.rtp) {
		...
		// Retrieve the codec RTX payload.
		for (const fmtp of m.fmtp) {
			if (fmtp.config === `apt=${codecPayload}`) {
				rtxPayload = fmtp.payload;
				break;
			}
		}

		// Remove the codec and RTX payload from fmtp.
		m.fmtp = m.fmtp.filter(fmtp => {
			return fmtp.payload !== codecPayload && fmtp.payload !== rtxPayload;
		});

		// Remove the codec rtcp feedback.
		m.rtcpFb = m.rtcpFb?.filter(rtcpFb => rtcpFb.payload !== codecPayload);

		// Remove the codec and RTX payloads.
		m.payloads = m.payloads
			?.split(' ')
			.filter(payload => {
				return (
					payload !== String(codecPayload) && payload !== String(rtxPayload)
				);
			})
			.join(' ');

		// Remove the codec and RTX from rtp collection.
		m.rtp = m.rtp.filter(rtp => {
			return rtp.payload !== codecPayload && rtp.payload !== rtxPayload;
		});
	}
}
